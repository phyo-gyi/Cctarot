...
RITUAL_AUDIO_PATH = Path(
    os.getenv(
        "RITUAL_AUDIO_PATH",
        str(BASE_DIR / "ritualaudio.ogg"),
    )
)
RITUAL_AUDIO_ALTERNATE_PATH = BASE_DIR / "ritualaudio.oga"
...
def send_ritual_audio(chat_id: int) -> None:
    """Send the supplied OGG/OGA file as a Telegram voice message."""
    audio_path = RITUAL_AUDIO_PATH
    if not audio_path.is_file() and RITUAL_AUDIO_ALTERNATE_PATH.is_file():
        audio_path = RITUAL_AUDIO_ALTERNATE_PATH

    if not audio_path.is_file():
        logger.warning("Ritual audio was not found: %s", audio_path)
        bot.send_message(
            chat_id,
            "အသံဖိုင် မတွေ့ပါ။ ritualaudio.ogg သို့မဟုတ် ritualaudio.oga ကို app.py နဲ့အတူ ထည့်ပြီး ပြန် run ပေးပါ။",
        )
        return
